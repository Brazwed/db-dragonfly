...
wait_healthy() {
    local container=$($COMPOSE ps -q dragonfly 2>/dev/null)
...
usage() {
    echo "Usage: $0 {up|down|restart|logs|status|shell|clean}"
    echo ""
...
cmd_up() {
    ...
    if wait_healthy; then
        echo "[$DB_NAME] running on port $DF_PORT"
    else
